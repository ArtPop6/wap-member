      ...
      <van-cell :key="index" v-for="(order, index) in orders">
        <div class="order-group">
          <div class="order-group__item">
            ...
            <div :key="index" class="card" v-for="(card, index) in order.order_items">
              <div class="card-pic">
                <van-image :src="card.product.cover_img_url" class="card-pic__url" fit="fill" radius="7">
              ...
              <div @click="gotoOrder(order.order_number)" class="card__desc">
                <div class="card-name">
                  ...
                  <span :class="[typeColor(card)]" class="type-time">{{card.product.category.name}}</span>
                </div>
                <div class="card-site">{{card.product.sub_title}}</div>
                <div class="card__price">
                  <div class="pay-price">￥{{(card.total_price).toFixed(2)}}</div>
...
export default {
  ...
  methods: {
    ...
    // gotoCardDetail(sku, product, store) {
    //   go(`./cardDetails.html?sku_id=${sku}&product_id=${product}&store=${store}`)
    // },
    getOrderInfo() {
      const data = {
        page: this.page,
