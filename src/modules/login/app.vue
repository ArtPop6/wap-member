<template>
  <div class="new-login">
    <div class="logo">
      ...
      <img class="logo-img" :src="companyLogo"/>
    </div>
...
export default {
  ...
  methods: {
    ...
    getLogo() {
      this.$http.get(`./corporations/logo?mac_code=${this.macCode}`).then(res => {
  ...
  mounted() {
    ...
    if (_isVaild(this.macCode)) {
      ...
      const weixinAuth = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=${this.appId}&redirect_uri=${target}&response_type=code&scope=snsapi_userinfo&state=${this.appId}`
