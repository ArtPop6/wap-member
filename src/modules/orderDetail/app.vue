  ...
  <div class="order-wrap">
    ...
    <div class="order-detail">
      ...
      <div :key="index" class="card-info" v-for="(card, index) in cards">
        ...
        <div class="card-desc">
          <div class="card-name">
            <span @click="gotoDetail(card)" class="type-name">{{card.product.name}}</span>
          ...
          <div class="card-num">
            <span class="pay-price">￥{{(card.total_price).toFixed(2)}}</span>
    ...
    <div class="pay-info" v-if="transacState.wait || transacState.close">
      <div class="sum-price">
        合计:
      ...
      <van-button
        ...
        type="primary"
      >{{transacState.close ? '再次购买' : '立即购买'}}</van-button>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    getOrderInfo() {
      this.$http
        .get(`./orders/cards/${this.orderNumber}`)
        .then(res => {
